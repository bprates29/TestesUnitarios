...
class ServicoPedidoTest {
    ...
    @BeforeEach
    void setup() {
        dummyCliente = new Cliente("cliente Dummy", true);
    }

    @Test
    void deveCriarPedidoComClienteAtivo() {
        // Dummy: só para satisfazer o parametro, sem légica extra
        Cliente dummyCliente = new Cliente("cliente Dummy", true);
    ...
    @Test
    void deveChamarSalvarAoAtualizarStatus() {
        Pedido pedido = new Pedido(dummyCliente);
        PedidoRepositoryFake repositoryFake = new PedidoRepositoryFake();
